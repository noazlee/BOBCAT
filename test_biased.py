...
def load_model():
    ...
    checkpoint = torch.load("saved_models/bobcat_final_eedi-3_biirt-biased_10_20251029_213955.pt", map_location=device)
...
def pick_biased_samples(st_policy, batch, model, config, n_query):
    
    # Init student parameters for inner loop
    batch_size = len(batch['input_labels'])
    question_dim = config.get('question_dim', 1)
    new_params = [torch.zeros(batch_size, question_dim).to(device).requires_grad_(True)]
    ...
    for _ in range(n_query):
        ...
        # train_mask_sample.data contains the new questions we asked
        sampled_questions.append(train_mask_sample.data.cpu())
        ...
        # Compute loss and update student params
        res = model(batch, config)
        loss = res['train_loss']
        
        grads = torch.autograd.grad(loss, new_params, create_graph=False)
        new_params = [(new_params[i] - 0.001 * grads[i]) for i in range(len(new_params))]
        del grads
    
    # Set final train mask
    config['train_mask'] = env_states['train_mask']
    config['meta_param'] = new_params[0]
    
    # Stack and convert one-hot to indices
    # sampled_questions is now (10, 32, 948) in one-hot form
    sampled_questions = torch.stack(sampled_questions)  # (10, 32, 948)
    sampled_q_indices = torch.argmax(sampled_questions, dim=-1)  # (10, batch_32size)
    sampled_q_indices = sampled_q_indices.T.numpy()  # (10, 32)
    
    return config, sampled_q_indices

def run_biased_test(st_policy, batch, model, n_query=10, question_dim=1):
    batch_size = len(batch['input_labels'])
    ...
    config, sampled_questions = pick_biased_samples(st_policy, batch, model, config, n_query)
...
def test_model(st_policy, model, test_data, n_query=10, question_dim=1):
...
if __name__ == "__main__":
    ...
    # init policy
    betas = (0.9, 0.999)
    st_policy = StraightThrough(model_params.get("n_question", 10), model_params.get("n_question", 10),
                                model_params.get("policy_lr", 2e-3), betas)
    ...
    csv_filename = f'outputs/sampled_questions_eedi3_biased_{timestamp}.csv'
